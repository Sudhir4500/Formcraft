...
export async function POST(req: NextRequest) {
  ...
  const response = await djangoPost<DjangoRegisterData>('auth/register/', body)
  ...
  if (!access || !refresh) {
    console.error('[register] Django success but no tokens in response:', response.data)
    return NextResponse.json(
      { success: false, message: 'Registration failed: invalid server response', data: null, errors: null },
      { status: 500 }
    )
  }
